...
describe("測試第二列city,area,detail,值是否正常傳送", () => {
  it("測試city,area change後值是否正確", async () => {
    ...
    expect(wrapper.findAll('[data-test="address_city"]')[1].element.value).toBe(
      "台中市"
    );

    await wrapper.findAll('[data-test="address_area"]')[1].setValue("太平區");
    await wrapper.findAll('[data-test="address_area"]')[1].trigger("change");
    expect(wrapper.findAll('[data-test="address_area"]')[1].element.value).toBe(
      "太平區"
    );

    await wrapper
      .findAll('[data-test="address_detail"]')[1]
      .setValue("中山路一段22號");
    expect(
      wrapper.findAll('[data-test="address_detail"]')[1].element.value
    ).toBe("中山路一段22號");
  });
});
describe("點擊表單送出", () => {
  it("測試表單送出驗證是否出現", async () => {
    ...
    expect(wrapper.findAll('[data-test="address_errorCity"]')[0].exists()).toBe(
      true
    );
    expect(wrapper.findAll('[data-test="address_errorCity"]')[0].text()).toBe(
      "請選擇縣市"
    );
    expect(wrapper.findAll('[data-test="address_errorArea"]')[0].exists()).toBe(
      true
    );
    expect(wrapper.findAll('[data-test="address_errorArea"]')[0].text()).toBe(
      "請選擇區域"
    );
    expect(
      wrapper.findAll('[data-test="address_errorAddress"]')[0].exists()
    ).toBe(true);
    expect(
      wrapper.findAll('[data-test="address_errorAddress"]')[0].text()
    ).toBe("請輸入詳細地址");
  });
  ...
  it("測試第一列地址更換city是否會清空地址Detail與驗證", async () => {
    ...
    expect(wrapper.vm.oTwAddress.addressList[0].sErrorAddressMessage).toBe("");
  });
});
